...
class Vak(object):
    ...
    def vanVakNaarActiviteit(self):
        ...
        if self.hc > 0:
            i = self.hc
            while(i != 0):
                activiteitenLijst.append(ActiviteitKlasse.Activiteit(i, 0, self.id,
                                         1000, self.aantalStudenten, self.studenten))
                i -= 1

        # werkcollege naar activiteiten
        if self.maxWc < self.aantalStudenten and self.wc > 0:
            studPerWc = self.studentenSplitsen( self.aantalStudenten, self.maxWc,
                                                self.studenten)
            i = 1
            for stud in studPerWc:
                activiteitenLijst.append(ActiviteitKlasse.Activiteit(i, 1, self.id,
                                         self.maxWc,len(stud), stud))
                i += 1
        elif self.wc > 0:
            activiteitenLijst.append(ActiviteitKlasse.Activiteit(1, 1, self.id,
                                     self.maxWc, self.aantalStudenten, self.studenten))

        # practicum naar activiteiten
        if self.maxPrac < self.aantalStudenten and self.prac > 0:
            studPerPrac = self.studentenSplitsen(self.aantalStudenten, self.maxPrac,
                                                 self.studenten)
            i = 1
            for stud in studPerPrac:
                activiteitenLijst.append(ActiviteitKlasse.Activiteit(i, 2, self.id,
                                         self.maxPrac,len(stud), stud))
                i += 1
        elif self.prac > 0:
            activiteitenLijst.append(ActiviteitKlasse.Activiteit(1, 2, self.id,
                                     self.maxPrac, self.aantalStudenten, self.studenten))
